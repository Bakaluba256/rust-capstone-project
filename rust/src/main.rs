...
fn main() -> bitcoincore_rpc::Result<()> {
    ...
    // Mine blocks to get spendable balance
    ...
    // Confirm transaction is in mempool
    ...
    let block_hash: BlockHash = tx.info.blockhash.expect("Expected block hash");
    let block_info = rpc.get_block_header_info(&block_hash)?;
    let block_height = block_info.height;

    // Extract miner input details
    let input_txid = decoded.vin[0].txid.as_ref().expect("Missing input txid");
    let input_vout = decoded.vin[0].vout.expect("Missing input vout");
    ...
    // Write output to out.txt for test suite
